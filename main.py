...
MAPPING_ACTIONS = {
    ...
    "15": "map_pan_tilt_speed_shortcut",
    "16": "map_zoom_speed_shortcut",
    "17": "map_adaptive_speed_toggle"
}

# Slider-specifieke mappings (worden toegevoegd aan MAPPING_ACTIONS)
SLIDER_MAPPING_ACTIONS = {
    "18": "map_zoom_speed_slider",
    "19": "map_pan_tilt_speed_slider",
    "20": "map_transition_speed_slider", 
    "21": "map_zoom_position_slider"
}

# Voeg slider mappings toe aan MAPPING_ACTIONS
MAPPING_ACTIONS.update(SLIDER_MAPPING_ACTIONS)

# Definieer correlatie tussen slider mapping actie en settings sleutel
SLIDER_MAPPING_KEYS = {
    "map_zoom_speed_slider": "zoom_speed_absolute",
    "map_pan_tilt_speed_slider": "ptr_speed_absolute", 
    "map_transition_speed_slider": "transition_speed_absolute",
    "map_zoom_position_slider": "zoom_position_absolute"
}
        ...
        try:
            ...
            for apcr in settings.get("apcrs", []):
                if 'camid' not in apcr:
                    logging.warning("APCR entry without 'camid' field – skipped")
                    continue
            ...
            logging.error("Unexpected error in check_for_new_apcrs", exc_info=True)
            time.sleep(1.0)
...
def event_loop(settings):
    ...
    while running:
        ...
        for axis_idx in range(joystick.get_numaxes()):
            ...
            if abs(value) > 0.4 and abs(prev_axis_values[axis_idx]) <= 0.4:
                #  ❶ alleen bij een *nieuwe* uitslag (>0,4) ➜ debounce
                for key, mapping in device_mappings.items():
                    if not key.startswith("camid_"):
                        continue
                    ...
                    #  ❷ alleen opslaan als het echt een andere camid is
                    if settings["global_settings"].get("selected_camid") != camid_num:
                        settings["global_settings"]["selected_camid"] = camid_num
                        save_settings(settings)
                        print(f"Selected camid {camid_num}.")
                # END CamID‑blok
            ...
            if apcr is not None:  # Zorg dat we een actieve camera hebben
                all_axis_values = [joystick.get_axis(i) for i in range(joystick.get_numaxes())]

                # ------------------------------------------------------------------
                # Verwerk mappings van type "absolute_axis"
                # ------------------------------------------------------------------
                for axis_idx in range(joystick.get_numaxes()):
                    current_value = all_axis_values[axis_idx]

                    # ⛔  Sla deze as over als hij óók als pan/tilt/zoom‑besturing
                    #     is gemapt; anders krijg je “speed‑sliders” die meebewegen.
                    if find_action_for_axis(device_mappings, axis_idx) is not None:
                        continue

                    for key, mapping in device_mappings.items():
                        if (
                            isinstance(mapping, dict)
                            and mapping.get("type") == "absolute_axis"
                            and mapping.get("index") == axis_idx
                        ):
                            # Bereken percentage
                            min_val = mapping.get("min", -1.0)
                            max_val = mapping.get("max", 1.0)

                            if abs(max_val - min_val) < 1e-6:
                                continue  # deling door nul voorkomen

                            # Normaliseer naar 0‑1
                            norm = (
                                (current_value - min_val) / (max_val - min_val)
                                if max_val > min_val
                                else (min_val - current_value) / (min_val - max_val)
                            )
                            norm = max(0.0, min(1.0, norm))
                            percentage = int(round(norm * 100))

                            # Pas instelling toe
                            handle_absolute_setting(key, percentage, apcr, settings)

                # ------------------------------------------------------------------
                # Verwerk mappings van type "virtual_axis"
                # ------------------------------------------------------------------
                for key, mapping in device_mappings.items():
                    if isinstance(mapping, dict) and mapping.get("type") == "virtual_axis":
                        axis_0 = mapping.get("axis_0")
                        axis_1 = mapping.get("axis_1")
                        value_0 = mapping.get("value_0")
                        value_1 = mapping.get("value_1")

                        # Controleer op geldige assen
                        if (
                            axis_0 is None
                            or axis_1 is None
                            or axis_0 >= len(all_axis_values)
                            or axis_1 >= len(all_axis_values)
                        ):
                            continue

                        # ⛔  Skip als één van de twee assen al voor bewegen is gemapt
                        if (
                            find_action_for_axis(device_mappings, axis_0) is not None
                            or find_action_for_axis(device_mappings, axis_1) is not None
                        ):
                            continue

                        # Huidige waarden ophalen
                        current_value_0 = all_axis_values[axis_0]
                        current_value_1 = all_axis_values[axis_1]

                        # Afstand berekenen
                        calib_distance = abs(value_1 - value_0)
                        if calib_distance < 1e-6:
                            continue  # deling door nul voorkomen

                        current_distance = abs(current_value_1 - current_value_0)
                        percentage = int(round(100 * current_distance / calib_distance))
...
def call_non_continuous_action(map_name, apcr, settings):
    ...
    elif map_name == "map_pan_tilt_speed_shortcut":
...
def choose_mapping_action(settings, device_id):
    ...
    # Functie om mapping string te maken voor een gegeven mapping
    def get_mapping_str(mapping):
        if not isinstance(mapping, dict):
            return "[unknown format]"
            
        if mapping["type"] == "button":
            return f"[button {mapping['index']}]"
        elif mapping["type"] == "axis":
            return f"[axis {mapping['index']} {mapping['direction']}]"
        elif mapping["type"] == "absolute_axis":
            return f"[absolute_axis {mapping['index']} 0-100]"
        elif mapping["type"] == "virtual_axis":
            return f"[absolute_axis {mapping['axis_0']} 0-49, absolute_axis {mapping['axis_1']} 50-100]"
        return "[unknown type]"

    # Toon reguliere mappings
    for k, v in MAPPING_ACTIONS.items():
        mapping_str = ""
        
        # Voor normale mappings
        if k in dev_map:
            mapping = dev_map[k]
        ...
        # ALLEEN voor slider mappings, toon de corresponderende absolute settings
        if v in SLIDER_MAPPING_KEYS:
            abs_key = SLIDER_MAPPING_KEYS[v]
            if abs_key in dev_map:
                mapping_str = get_mapping_str(dev_map[abs_key])
                
        print(f"{k}. {v} {mapping_str}")

    # Menu-opties voor CamID en terug
    slider_count = len(SLIDER_MAPPING_ACTIONS)
    camid_option = str(17 + slider_count + 1)  # Na de slider opties
    return_option = str(17 + slider_count + 2)  # Na de camid_menu optie
    
    print(f"{camid_option}. Map CamID to button")
    print(f"{return_option}. return")

    while True:
        choice = input("> ").strip().lower()
        if choice == return_option or choice == "return":
            return None
        if choice == camid_option:
            return "camid_menu"  # speciaal signaal
        if choice in MAPPING_ACTIONS:
            return choice
        print("Invalid choice, try again.")

def wait_for_mapping_input(device, buttons_only=False):
    """
    Wacht tot de gebruiker een knop indrukt of een as beweegt om een actie te mappen.
    
    Args:
        device: The input device (joystick)
        buttons_only: If True, only detect button input (no axes)
        
    Returns:
        tuple: (input_type, input_idx, [direction]) or None if canceled
    """
    ...
    # Wacht kort zodat alles kan stabiliseren
    pygame.event.pump()
    time.sleep(0.5)
    
    # Lees initiële waarden
    pygame.event.pump()
    initial_axis_values = [device.get_axis(i) for i in range(device.get_numaxes())]
    
    # Hogere threshold voor de eerste bewegingsdetectie om ruis te negeren
    ...
    clock = pygame.time.Clock()
    input_buffer = ""
    button_presses = {i: False for i in range(device.get_numbuttons())}

    while True:
        pygame.event.pump()

        # Check axes (alleen als buttons_only=False)
        if not buttons_only:
            for axis_idx in range(device.get_numaxes()):
                value = device.get_axis(axis_idx)
                # Vergelijk met initiële waarde bij programma start
                delta = abs(value - initial_axis_values[axis_idx])
                
                # Alleen updates verwerken als de verandering significant is 
                # EN we nog geen keuze hebben gemaakt of een andere as bewegen
                if delta > initial_threshold and (chosen_input is None or chosen_input[0] != "axis" or chosen_input[1] != axis_idx):
                    direction = "positive" if value > 0 else "negative"
                    new_input = ("axis", axis_idx, direction)
                    chosen_input = new_input
                    print_selected_input(chosen_input)

        # Check buttons - alleen detecteren bij eerste indrukken (niet bij vasthouden)
        for btn_idx in range(device.get_numbuttons()):
            pressed = device.get_button(btn_idx)
            if pressed and not button_presses[btn_idx]:
                button_presses[btn_idx] = True
                new_input = ("button", btn_idx)
                chosen_input = new_input
                print_selected_input(chosen_input)
            elif not pressed:
                button_presses[btn_idx] = False
...
# Voeg dit bovenaan toe, bij de andere globale variabelen
SLIDER_THRESHOLD = 1  # Minimaal benodigd verschil om als werkelijke verandering te zien
_last_slider_values = {}  # Dict om laatste waarden bij te houden

def handle_absolute_setting(key, percentage, apcr, settings):
    """
    Verwerk een absolute instelling op basis van het percentage.
    
    Args:
        key: De sleutel van de instelling (bijv. 'zoom_position_absolute')
        percentage: Het berekende percentage (0-100)
        apcr: De actieve APC-R configuratie
        settings: De applicatie-instellingen
    """
    global _last_slider_values
    
    # Begrens percentage voor veiligheid
    percentage = max(1, min(100, percentage))
    
    # Debounce: alleen verwerken als we een significante verandering zien sinds laatste waarde
    last_value = _last_slider_values.get(key, None)
    if last_value is not None and abs(percentage - last_value) <= SLIDER_THRESHOLD:
        # Kleine wijziging binnen threshold - negeren om ruis te voorkomen
        return
    
    # Bijwerken van laatste waarde
    _last_slider_values[key] = percentage
    
    # Sla huidige instellingswaarde op voor vergelijking
    changed = False
    
    if key == "ptr_speed_absolute":
        # Update alleen als de waarde significant is veranderd
        current = settings["global_settings"].get("ptr_speed", 100)
        if abs(current - percentage) > SLIDER_THRESHOLD:
            settings["global_settings"]["ptr_speed"] = percentage
            changed = True
            if debug_mode:
                print(f"[DEBUG] PTR speed ingesteld op {percentage}% via slider")
    
    elif key == "zoom_speed_absolute":
        current = settings["global_settings"].get("zoom_speed", 100)
        if abs(current - percentage) > SLIDER_THRESHOLD:
            settings["global_settings"]["zoom_speed"] = percentage
            changed = True
            if debug_mode:
                print(f"[DEBUG] Zoom speed ingesteld op {percentage}% via slider")
    
    elif key == "transition_speed_absolute":
        current = settings["global_settings"].get("preset_transition_speed", 100)
        if abs(current - percentage) > SLIDER_THRESHOLD:
            settings["global_settings"]["preset_transition_speed"] = percentage
            changed = True
            if debug_mode:
                print(f"[DEBUG] Transition speed ingesteld op {percentage}% via slider")
    
    elif key == "zoom_position_absolute":
        # Bereken en stuur absolute zoom-positie
        # Van percentage (0-100) naar zoom waarde (0-4095)
        zoom_value = int(round((percentage / 100.0) * 4095))
        ...
        if current_time - last_zoom_time > 0.1:  # Max 10 updates per seconde
            ...
            if debug_mode:
                print(f"[DEBUG] Absolute zoom positie: {percentage}% -> {zoom_value}")
            
            # Bouw en stuur absolute zoom pakketje
            camid = apcr.get('camid', 1)
            zoom_packet = build_absolute_zoom_packet(camid, zoom_value)
            send_apcr_command(apcr, bytes.fromhex(zoom_packet))
    
    # Sla de instellingen op als we een instelling hebben gewijzigd (niet voor zoom positie)
    if changed and key != "zoom_position_absolute":
        ...
        if current_time - last_save_time > 0.1:  # Max 100x per seconde opslaan
            ...
            # Stuur status update naar Companion clients
            try:
                import interpreter
                interpreter.trigger_status_update()
...
def build_absolute_zoom_packet(camid, zoom_val):
    """
    Bouw een absoluut zoom pakketje voor de opgegeven zoom-waarde.
    Gebruikt in absolute zoom mapping.
    
    Args:
        camid: Camera ID
        zoom_val: Zoom waarde (0-4095)
        
    Returns:
        str: Hex string van het pakketje
    """
    # Begrens zoom waarde
    z = max(0, min(4095, int(round(zoom_val))))
    lo = z & 0xFF
    hi = (z >> 8) & 0xFF
    packet = f"0A{camid:02X}0600000E0A80" + f"00{lo:02X}{hi:02X}"
    if debug_mode:
        print(f"[DEBUG] Absolute zoom pakketje: zoom_val={z}, Packet={packet}")
    return packet
...
def map_device_action(settings, device, action):
    ...
    Returns:
        bool: True if successful, False otherwise
    """
    # Altijd action naar string converteren voor consistentie
    action_str = str(action)
    
    # Get the mapping name for this action
    map_name = MAPPING_ACTIONS.get(action_str)
   
    # Check of het een slider mapping is (alleen voor de specifieke opties 18-21)
    if action_str in ["18", "19", "20", "21"]:
        # Gebruik de speciale slider kalibratie functie
        return calibrate_slider_mapping(settings, device, action_str, map_name)
   
    # Bepaal of we alleen knoppen willen detecteren voor deze actie
    buttons_only = map_name in [
        "map_recenter", "map_active_track",
        "map_pan_tilt_speed_increase", "map_pan_tilt_speed_decrease",
        "map_zoom_speed_increase", "map_zoom_speed_decrease",
        "map_pan_tilt_speed_shortcut", "map_zoom_speed_shortcut",
        "map_adaptive_speed_toggle"
    ]
    
    # Debug print
    print(f"Gebruik reguliere mapping voor {action_str}: {map_name}")
    
    # Originele code voor normale mappings
    chosen_input = wait_for_mapping_input(device, buttons_only=buttons_only)
    if chosen_input is None:
    ...
    # Add the new mapping
    if allows_multiple:
        if action_str not in current_mappings or not isinstance(current_mappings[action_str], list):
            current_mappings[action_str] = []
        
        if any(
            m["type"] == new_mapping["type"] and m["index"] == new_mapping["index"] and
            m.get("direction") == new_mapping.get("direction")
            for m in current_mappings[action_str]
        ):
            print(f"This input is already mapped for '{map_name}'.")
        else:
            current_mappings[action_str].append(new_mapping)
            print(f"Added new mapping for '{map_name}': {new_mapping}")
    else:
        current_mappings[action_str] = new_mapping
...
def calibrate_slider_mapping(settings, device, action_key, map_name):
    """
    Kalibreer een slider-mapping voor het opgegeven apparaat.
    
    Args:
        settings: Application settings
        device: The input device (joystick)
        action_key: The action key in MAPPING_ACTIONS
        map_name: The mapping name (e.g., "map_zoom_speed_slider")
    
    Returns:
        bool: True if the calibration was successful
    """
    # Get device ID
    device_id = device.get_guid() if hasattr(device, 'get_guid') else device.get_name()
    
    print(f"\nKalibratie voor {map_name}")
    print("We gaan de minimale (0%) en maximale (100%) posities van de slider kalibreren.")
    
    # ----- 0% positie -----
    print("\nStap 1: Detecteer 0% positie")
    print("Beweeg de as/slider die je wilt gebruiken voor de 0% positie.")
    
    axis_0 = wait_for_axis_movement(device)
    if axis_0 is None:
        print("Kalibratie geannuleerd.")
        return False
    
    print(f"As {axis_0} geselecteerd voor 0% positie.")
    input("Zet de as/slider nu op de gewenste 0% positie en druk op Enter...")
    
    # Lees huidige waarde
    pygame.event.pump()
    value_0 = device.get_axis(axis_0)
    print(f"0% waarde vastgelegd: {value_0:.3f}")
    
    # ----- 100% positie -----
    print("\nStap 2: Detecteer 100% positie")
    print("Beweeg de as/slider die je wilt gebruiken voor de 100% positie.")
    print("Dit kan dezelfde as zijn of een andere.")
    
    # Kleine rustpauze om onbedoelde detectie te voorkomen
    time.sleep(1.5)
    
    axis_1 = wait_for_axis_movement(device)
    if axis_1 is None:
        print("Kalibratie geannuleerd.")
        return False
    
    print(f"As {axis_1} geselecteerd voor 100% positie.")
    input("Zet de as/slider nu op de gewenste 100% positie en druk op Enter...")
    
    # Lees huidige waarde
    pygame.event.pump()
    value_1 = device.get_axis(axis_1)
    print(f"100% waarde vastgelegd: {value_1:.3f}")
    
    # ----- Mapping maken en opslaan -----
    # Maak passende sleutel op basis van de mapping naam
    if map_name == "map_zoom_speed_slider":
        setting_key = "zoom_speed_absolute"
    elif map_name == "map_pan_tilt_speed_slider":
        setting_key = "ptr_speed_absolute"
    elif map_name == "map_transition_speed_slider":
        setting_key = "transition_speed_absolute"
    elif map_name == "map_zoom_position_slider":
        setting_key = "zoom_position_absolute"
    else:
        setting_key = map_name.replace("map_", "") + "_absolute"
    
    # Maak mapping afhankelijk van of het één as is of twee verschillende assen
    if axis_0 == axis_1:
        mapping_entry = {
            "type": "absolute_axis",
            "index": axis_0,
    ...
    # Zorg dat device entries bestaan
    if "devices" not in settings:
        settings["devices"] = {}
    if device_id not in settings["devices"]:
    ...
    # Sla mapping op
    settings["devices"][device_id][setting_key] = mapping_entry
    save_settings(settings)
    
    print(f"\nMapping '{setting_key}' succesvol gekalibreerd en opgeslagen.")
    print("Je kunt de slider nu gebruiken om de waarde direct in te stellen.")
    
    return True


def wait_for_axis_movement(device, threshold=0.5, timeout=30):
    """
    Wacht tot er beweging wordt gedetecteerd op een as.
    
    Args:
        device: The input device (joystick)
        threshold: Minimum waarde die als beweging wordt gezien
        timeout: Maximale wachttijd in seconden
        
    Returns:
        int: Index van de as die bewoog, of None bij timeout/annulering
    """
    print("Beweeg de gewenste as/slider...")
    print("Typ 'cancel' en druk op Enter om te annuleren.")
    
    # Lees beginwaardes
    pygame.event.pump()
    initial_values = [device.get_axis(i) for i in range(device.get_numaxes())]
    
    # Setup voor non-blocking input
    input_buffer = ""
    start_time = time.time()
    ...
    while time.time() - start_time < timeout:
        ...
        # Check voor as-beweging
        for i in range(device.get_numaxes()):
            current_value = device.get_axis(i)
            delta = abs(current_value - initial_values[i])
            
            if delta > threshold:
                print(f"Beweging gedetecteerd op as {i}!")
                return i
        
        # Check voor toetsenbordinvoer (non-blocking)
        if msvcrt.kbhit():
            char = msvcrt.getwche()
            if char == '\r':  # Enter pressed
                if "cancel" in input_buffer.lower():
                    print("Kalibratie geannuleerd.")
                    return None
                input_buffer = ""
    ...
    print("Timeout - geen as-beweging gedetecteerd.")
    return None
...
def send_position_requests(settings):
    """
    Stuur *altijd* een 08‑positiequery naar de geselecteerde camID
    en daarnaast (tijdens startup) naar niet‑geïnitialiseerde camera’s.
    """
    while True:
        try:
            apcrs = settings.get("apcrs", [])
            ...
            # 1) eerst de geselecteerde camID – altijd
            ...
            # 2) daarnaast alle camera’s die nog geen FDB‑update kregen
            for apcr in apcrs:
                if apcr.get("camid") not in _initialized_apcrs:
                    _send_single_position_request(apcr)

            time.sleep(settings["global_settings"].get(
                        "position_request_frequency", 1.2))
        except Exception as e:
            logging.error(f"[ERROR] position request loop: {e}")
            time.sleep(1.0)
